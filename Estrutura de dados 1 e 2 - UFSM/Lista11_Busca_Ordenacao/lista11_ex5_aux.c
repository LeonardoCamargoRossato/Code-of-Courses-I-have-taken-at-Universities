...
static int compara_salario(Funcionario** v,int j){
  float salario1, salario2;
  //for(int j = 0; j<n; j++){
    salario1 = (v[j]->horas_mes)*(v[j]->valor_hora);
    salario2 = (v[j+1]->horas_mes)*(v[j+1]->valor_hora);
    printf("%s|h_mes: %d pay1: %.2f R$ || %s|h_mes: %d pay2: %.2f R$\n",
      v[j]->nome,v[j]->horas_mes,salario1,v[j+1]->nome,v[j+1]->horas_mes,salario2);
...
void imprime_marajas(int n, Funcionario** v){
  ...
  for(i = n-1; i>0; i--){
    troca = 0;
    for(j = 0; j<i; j++){
      if(compara_salario(v,j) > 0){ //MAIOR Q ZERO, TROCA
        temp = j;  // AONDE TROCA
